...
func (d Decimal) string(trimTrailingZeros bool) string {
	...
	dExpInt := int(d.exp)
	if len(b) > -dExpInt {
		sb.Write(b[:len(b)+dExpInt])
		fractionalPart = b[len(b)+dExpInt:]
	} else {
		sb.WriteByte('0')

		num0s := -dExpInt - len(b)

		fractionalPart = make([]byte, num0s+len(b))
		i := copy(fractionalPart, bytes.Repeat([]byte{'0'}, num0s))
		copy(fractionalPart[i:], b)
	}
	...
	if len(fractionalPart) > 0 {
		sb.Grow(len(fractionalPart))
		sb.WriteByte('.')
		sb.Write(fractionalPart)
	}
